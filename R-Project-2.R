...
time <- difftime(Sys.time(), dates_as_dates[1], units = "weeks")

print(paste(
  round(time, digits = 2),
  "weeks has passed since date represented in the dates vector."
))
...
df <-
  data.frame(
    name = c("Arsenal", "Chelsea", "Man.City", "Liverpool"),
    position = c(5, 4, 2, 1),
...
subtract_dates <- function(date1, date2 = Sys.Date()) {
  return(as.numeric(difftime(date2, date1, units = "days")))
}
...
### Answer to Excercise 6:
library(data.table)

dat <- data.table(iris)["Petal.Length" > 1.5, lapply(.SD, median), by = Species, .SDcols = -"Petal.Length"]
dat
...
### Answer to Excercise 8:
source(
  'C:/Users/Sarang.Zendehrooh/Desktop/IE stuff/Statistical Programming - R/indv-work/quiz2/auxiliary_functions.R'
)

as.list(body(count_greater_than))
body(count_greater_than)[[2]] <- substitute(ret <- sum(x > 1))

gr_1 <-
  as.data.table(iris)[, sapply(.SD, count_greater_than), .SDcols = which(sapply(iris, is.numeric))]
...
### Answer to Excercise 9:
flights <-
  as.data.table(
    readRDS(
      "C:/Users/Sarang.Zendehrooh/Desktop/IE stuff/Statistical Programming - R/indv-work/quiz2/userbase.RData"
    )
  )

top3 <-
  flights[sale_channel == "online" &
            price > 50 , lapply(.SD, mean), by = "destination", .SDcols = "price"][order(-price), head(destination, 3)]
top3
rm(list = ls())
...
### Answer to Excercise 10:
other_function <- function(...) {
  arg <- list(...)
  total_length <- sum(sapply(arg, function(x)
    sum(length(x))))
  return(total_length)
}
